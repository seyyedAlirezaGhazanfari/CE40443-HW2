...
def handle_share_request(sender_address, message, server_socket):
...
def handle_heartbeat():
    while True:
        ...
        for peer in alive_time_table.keys:
            if dt.now() - alive_time_table[peer] > dt.min(1):
                alive_time_table.pop(peer)
                for file in file_maps.keys:
                    for host in file_maps[file]:
                        if host == peer:
                            file_maps[file].remove(peer)
